...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        features: torch.nn.ModuleList = torch.nn.ModuleList([ConvBNActivation(3, input_channel,
    ...
    def _forward_impl(self, x: Tensor) -> List:
        ...
        for b in self.features:
            x = b(x)
            if isinstance(b, InvertedResidual):
                if b.conv[1].stride == 2:
                    outputs.append(x)
            elif isinstance(b, ConvBNActivation):
                outputs.append(x)
        return outputs
    ...
    def get_dimensions(self):
        ...
        for b in self.features:
            if isinstance(b, InvertedResidual):
                if b.conv[1].stride == 2:
                    dims.append(b.conv[-2].out_channels)
            elif isinstance(b, ConvBNActivation):
                dims.append(b[-3].out_channels)
        return dims
